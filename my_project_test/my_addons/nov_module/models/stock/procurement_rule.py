...
class ProcurementRule(models.Model):
    ...
    def _run_buy(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        for supp in self._make_po_select_supplier(values, suppliers):
            supplier = supp['supplier']
            partner = supp['supplier'].name
    ...
    def _make_po_select_supplier(self, values, suppliers):
        ...
        if product_supplier:
            if values['warehouse_id']['flag'] == True:
                ...
                for line in values['group_id']['sale_id'].order_line:
                    vals = {}
                    suppliers_list = []
                    for line_supplier in line.line_supplier_ids:
                        # print('line.line_supplier_ids.supplier_id',line.line_supplier_ids.supplier_id)
                        related_supplier = self.env['product.supplierinfo'].search([('name','=',line_supplier.supplier_id.id)],limit=1)
                        if related_supplier:
                            vals = {
                                'supplier':related_supplier,
                                'qty':line_supplier.qty
                            }
                        suppliers_list.append(vals)
